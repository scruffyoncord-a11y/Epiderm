...
AI_GENERATORS = re.compile(
    r"midjourney|dall[-\u00b7 ]?e|stable ?diffusion|firefly|imagen|sdxl|comfyui|automatic1111|novelai|leonardo\.ai|"
    r"nightcafe|dreamstudio|ideogram|\bflux\b|runway|\bsora\b|\bveo\b|gemini|invokeai|craiyon|bing image creator", re.I)
# PNG text chunks written by common image generators
AI_PNG_KEYS = {"parameters", "prompt", "workflow", "sd-metadata", "invokeai_metadata", "dream"}
# IANA digital-source-type values that declare AI involvement
AI_SOURCE_TOKENS = (b"trainedAlgorithmicMedia", b"compositeWithTrainedAlgorithmicMedia", b"algorithmicMedia")
...
def _extract_image(data: bytes, m: Metadata) -> None:
    ...
    info = {str(k).lower(): v for k, v in img.info.items() if isinstance(k, str)}
    m.has_any_metadata = m.has_any_metadata or any(isinstance(v, str) for v in info.values())  # PNG text chunks
    for key in sorted(AI_PNG_KEYS & info.keys()):
        m.ai_markers.append(f"contains '{key}' settings written by an AI image generator")
    # Only fields that name software. Free-text fields such as Description could mention "runway" or "gemini" innocently.
    tool_keys = ("software", "source", "generator", "creator", "tool", "producer", "generated by")
    text_blob = " ".join([software] + [str(info[k])[:300] for k in tool_keys if isinstance(info.get(k), str)])
    gen = AI_GENERATORS.search(text_blob)
    if gen:
        m.ai_markers.append(f"names an AI image tool: {gen.group(0)}")
    if any(tok in data for tok in AI_SOURCE_TOKENS):
        m.ai_markers.append("declares itself AI-generated in its content credentials")
    m.content_credentials = b"c2pa" in data or b"jumb" in data
...
def build_signals(m: Metadata, vendor: Optional[str] = None, now: Optional[datetime] = None) -> list[Signal]:
    ...
    if m.ai_markers:
        out.append(_sig("ai_generated_marker", "The image carries markers of AI generation", S, 0.8, 0.8, "; ".join(m.ai_markers)))
    elif m.content_credentials:
        out.append(_sig("content_credentials", "The image has content credentials (provenance data) that can be checked with a Content Credentials viewer",
                        N, 0.0, 0.5))
...
def analyze_document(data: bytes, filename: str = "", vendor: Optional[str] = None,
    ...
    if m.format == "image" and not m.unreadable:
        unknown.append("Whether the image is AI-generated or a deepfake: this only reads metadata, which is easy to remove "
                       "(social media strips it), so no marker does not mean the image is real")
